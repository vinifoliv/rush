...
class TesteCLI(ICLI):
    ...
    def _obter_servico(self):
        ...
        opcoes = {s.obter_id(): s.obter_nome() for s in servicos}
        self._console.menu(
        ...
        servico = next((s for s in servicos if s.obter_id() == servico_id), None)
        if not servico:
            raise ValueError("Serviço não existe.")
        return servico

    def _obter_rota(self, servico: Servico):
        rotas = self._buscar_rotas_por_servico_id_usecase.executar(servico.obter_id())
        opcoes = {r.obter_id(): r.obter_caminho() for r in rotas}
        self._console.menu(
