...
class Sessao:
    ...
    def obter_media(self):
        tempos_resposta = [b.obter_media() for b in self.batches]
        media = sum(tempos_resposta) / len(tempos_resposta)
        return media

    def obter_maximo(self):
        tempos_resposta = [b.obter_maximo() for b in self.batches]
        maximo = max(tempos_resposta)
        return maximo

    def obter_minimo(self):
        tempos_resposta = [b.obter_minimo() for b in self.batches]
